...
public class Generate extends Bill
{
   ...
   for(int i=1;i<=12;i++)
   {
	   ...
	   if(arr[i]==5)
	   {
		   System.out.println("  Veg Chopsouey             "+p[i]+"   Price :"+170*p[i]+" rs");
	   }
	   if(arr[i]==6)
	   {
		   System.out.println("  Chilly Paneer Gravy Price "+p[i]+"   Price :"+205*p[i]+" rs");
	   }
	   if(arr[i]==7)
	   { 
	   ...
	   if(arr[i]==11)
	   {
		   System.out.println("  Coffee Espresso           "+p[i]+"   Price :"+75*p[i]+" rs");
	   }
	   if(arr[i]==12)
	   {
